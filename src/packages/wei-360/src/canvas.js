export default function (imgArr, that, canvasDom, fun) {
  ...
  function playImages(images) {
    ...
    function draw() {
      ...
      ctx.drawImage(images[that.nowRota], 0, 0, canvas.height, canvas.width);
    }
    function tick() {
...
export function Test(imgArr, nowRota, canvasDom, fun) {
  //初始化参数
  var canvas = null;
  var ctx = null;
  var loadNum = 0;
  ...
  function playImages(images) {
    ...
    function draw() {
      ctx.clearRect(0, 0, canvas.height, canvas.width);
      // ctx.fillRect(0, 0, canvas.height, canvas.width);
      ctx.drawImage(images[nowRota.value], 0, 0, canvas.height, canvas.width);
    }
    function tick() {
      requestAnimationFrame(tick);
      now = Date.now();
